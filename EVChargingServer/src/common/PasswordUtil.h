...
//
// 背景(三个来源互相冲突):
//   1) 《CS通信协议.md》 7.1 强制: 客户端先 SHA-256, 服务端再生成随机盐,
//      存储 SHA256(客户端哈希 + Salt), 严禁明文。
//   2) 数据库端 Demo 现状: users.password / admins.password 直接明文存储,
//      userLogin / adminLogin 用 WHERE phone=? AND password=? 明文比对。
//   3) 项目决策: 本阶段先沿用明文以尽快打通链路, 后续再切换。
//
